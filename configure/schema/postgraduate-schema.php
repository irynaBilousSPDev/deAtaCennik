<?php
/**
 * JSON-LD EducationalOccupationalProgram for postgraduate singles.
 *
 * - optional schema_seo_description (manual SEO, overrides auto description)
 *
 * Template: template-parts/content/content-single-pg-mba.php (single-postgraduate.php)
 *
 ...
 * - contact via options: contact_postgraduate / contact_warsaw
 *
 * @package akademiata
 */
 ...
 */
function akademiata_build_postgraduate_program_schema($post_id) {
    $post_id = (int) $post_id;
    if (get_post_type($post_id) !== 'postgraduate') {
        return null;
    }
    ...
    $description = akademiata_schema_build_description($post_id, 'postgraduate');
    if ($description !== '') {
    ...
    $keywords = akademiata_schema_collect_keywords($post_id, 'postgraduate');
    if ($keywords !== array()) {
    ...
    $schema['programType']            = akademiata_schema_program_type_label('postgraduate');
    ...
    $contact_point = akademiata_schema_pg_mba_contact_point($post_id, 'postgraduate');
...
function akademiata_output_postgraduate_program_schema() {
    if (is_admin() || !is_singular('postgraduate')) {
        return;
    }

    akademiata_schema_output_json_ld(
        akademiata_build_postgraduate_program_schema((int) get_queried_object_id())
    );
}

add_action('wp_head', 'akademiata_output_postgraduate_program_schema', 20);
