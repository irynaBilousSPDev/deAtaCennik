...
function resolveWelyoRemotePath(themeRemotePath) {
	return themeRemotePath.replace(/\/themes\/[^/]+$/, '/welyo-config.php');
}

const WELYO_ENV_KEYS = [
	'BASE_URL',
	'LOGIN',
	'API_KEY',
	'CAMPAIGN_ID',
...
function welyoEnvValue(env, target, key) {
	const envKey = target === 'prod' ? `WELYO_PROD_${key}` : `WELYO_${key}`;
	const val = env[envKey];
	return val === undefined ? '' : String(val).trim();
}

function hasWelyoDeployConfig(env, target) {
	return WELYO_ENV_KEYS.some((key) => welyoEnvValue(env, target, key) !== '');
}

function phpDefineValue(raw) {
	if (/^-?\d+$/.test(raw)) {
		return raw;
	}
...
function buildWelyoConfigPhp(env, target) {
	if (!hasWelyoDeployConfig(env, target)) {
		return null;
	}

	const lines = [
		'<?php',
		'/** Generated by npm run deploy — source: deploy.local.env */',
		"if ( ! defined( 'ABSPATH' ) ) { exit; }",
		'',
	];

	for (const key of WELYO_ENV_KEYS) {
		const val = welyoEnvValue(env, target, key);
		if (!val) {
			continue;
		}
		lines.push(
			`if ( ! defined( 'WELYO_${key}' ) ) define( 'WELYO_${key}', ${phpDefineValue(val)} );`
		);
	}
...
async function uploadWelyoConfig(sftp, env, target, themeRemotePath, dryRun) {
	const content = buildWelyoConfigPhp(env, target);
	if (!content) {
		return false;
	}

	const remoteFile = resolveWelyoRemotePath(themeRemotePath);
	if (dryRun) {
		console.log(`[dry-run] welyo-config.php → ${remoteFile}`);
		return true;
	}

	await ensureRemoteDir(sftp, path.posix.dirname(remoteFile));
	process.stdout.write(`↑ welyo-config.php → ${remoteFile}\n`);
...
async function main() {
	...
	try {
		...
		welyoUploaded = await uploadWelyoConfig(sftp, env, TARGET, remotePath, dryRun);
	} finally {
		if (!dryRun) {
			await sftp.end();
		}
	...
	if (welyoUploaded) {
		console.log(`Remote welyo: ${resolveWelyoRemotePath(remotePath)}`);
	}
}
